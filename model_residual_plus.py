...
class GResidual(nn.Module):
  def __init__(self, block, layers, image_channels, num_bins):
    ...
    self.left_pfc = nn.Linear(512 * block.expansion, 512)
    self.left_yfc = nn.Linear(512 * block.expansion, 512)

    self.right_pfc = nn.Linear(512 * block.expansion, 512)
    self.right_yfc = nn.Linear(512 * block.expansion, 512)
  ...
  def forward(self, x1, x2, x3):
    ...
    p_left = self.left_pfc(left)
    y_left = self.left_yfc(left)

    p_right = self.right_pfc(right)
    y_right = self.right_yfc(right)
