...
def parse_args():
  ...
  parser.add_argument(
    '--alpha', dest='alpha', help='Regression loss coefficient.', default=2, type=float
  )
  parser.add_argument(
...
def get_pre_facefc_params(model):
  #Generator function that yields fc layer params.
  b = [model.p_face_fc, model.y_face_fc]
  for i in range(len(b)):
    for module_name, module in b[i].named_modules():
      if 'bn' in module_name:
        module.eval()
...
def get_pre_eyefc_params(model):
  #Generator function that yields fc layer params.
  b = [model.p_eye_fc, model.y_eye_fc]
  for i in range(len(b)):
    for module_name, module in b[i].named_modules():
      if 'bn' in module_name:
        module.eval()
...
def get_eyefc_params(model):
  #Generator function that yields fc layer params.
  b = [model.eyefc_pitch, model.eyefc_yaw]
  for i in range(len(b)):
    for module_name, module in b[i].named_modules():
      if 'bn' in module_name:
        module.eval()
...
def get_pitch_atten_params(model):
  b = [model.p_W1, model.p_W2]
  for i in range(len(b)):
    for module_name, module in b[i].named_modules():
      if 'bn' in module_name:
        module.eval()
...
def get_yaw_atten_params(model):
  b = [model.y_W1, model.y_W2]
  for i in range(len(b)):
    for module_name, module in b[i].named_modules():
      if 'bn' in module_name:
        module.eval()
...
if __name__=='__main__':
  ...
  if dataset=="gaze360":
    ...
    summary_name = '{}_{}'.format('AttnNet-gaze360', str(today.strftime('%Y-%-m*%-d_%-H*%-M*%-S')))
    ...
    optimizer_gaze = torch.optim.Adam([
      ...
      {'params' : get_pre_facefc_params(model), 'lr' : args.lr},
      {'params' : get_pre_eyefc_params(model), 'lr' : args.lr},
      {'params' : get_facefc_params(model), 'lr' : args.lr},
      {'params' : get_eyefc_params(model), 'lr' : args.lr},
      {'params' : get_pitch_atten_params(model), 'lr' : args.lr},
      {'params' : get_yaw_atten_params(model), 'lr' : args.lr},
      {'params' : [model.p_v], 'lr' : args.lr},
      {'params' : [model.y_v], 'lr' : args.lr}
    ], lr = args.lr)
    ...
    with open(os.path.join(valpath, dataset+".log"), 'w') as outfile:
      ...
      for epoch in range(num_epochs):
        ...
        for i, (face, left, right, labels, cont_labels, name) in enumerate(train_loader):
          ...
          #print(gb_pitch, gb_yaw, gr_pitch, gr_yaw)
          ...
          #print(pre_gb_pitch, pre_gb_yaw, pre_gr_pitch, pre_gr_yaw)
          #ok
