...
if __name__=='__main__':
  ...
  if dataset=="gaze360":
    ...
    optimizer_gaze = torch.optim.Adam([
      ...
      {'params' : get_atten_params(model), 'lr' : args.lr/10},
      {'params' : [model.v], 'lr' : args.lr/10}
    ], lr = args.lr)
    ...
    with open(os.path.join(valpath, dataset+".log"), 'w') as outfile:
      ...
      for epoch in range(num_epochs):
        ...
        for i, (face, left, right, labels, cont_labels, name) in enumerate(train_loader):
          ...
          g_pitch, g_yaw = gb_pitch+gr_pitch, gb_yaw+gr_yaw

          #Cross Entropy Loss
          cross_gb_pitch = criterion(gb_pitch, label_pitch)
          cross_gb_yaw = criterion(gb_yaw, label_yaw)
          cross_g_pitch = criterion(g_pitch, label_pitch)
          cross_g_yaw = criterion(g_yaw, label_yaw)
