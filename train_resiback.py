...
def parse_args():
  ...
  parser.add_argument(
    '--beta', dest='beta', help='Regression loss coefficient.', default=2, type=float
  )
  parser.add_argument(
...
if __name__=='__main__':
  ...
  if dataset=="gaze360":
    ...
    if args.snapshot == '':
      face = model.face_res
      left = model.left_res
      right = model.right_res
      load_filtered_state_dict(face, model_zoo.load_url(pre_url))
      load_filtered_state_dict(left, model_zoo.load_url(pre_url))
      load_filtered_state_dict(right, model_zoo.load_url(pre_url))
    else:
    ...
    optimizer_gaze = torch.optim.Adam([
      ...
      {'params' : get_ignored_params(model.left_res), 'lr' : 0},
      {'params' : get_ignored_params(model.right_res), 'lr' : 0},
      ...
      {'params' : get_eyesfc_params(model), 'lr' : args.lr},
    ...
    with open(os.path.join(valpath, dataset+".log"), 'w') as outfile:
      ...
      for epoch in range(num_epochs):
        ...
        for i, (face, left, right, labels, cont_labels, name) in enumerate(train_loader):
          ...
          f_loss_yaw = criterion(face_yaw, label_yaw)
          ...
          f_loss_gb_pitch = reg_criterion(pre_gb_pitch, label_pitch_cont)
