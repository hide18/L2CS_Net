      ...
      for epochs in folder:
        ...
        with torch.no_grad():
          for j, (face, left, right, lebls, cont_labels, name) in enumerate(test_loader):
            ...
            pre_gb_pitch = torch.sum(pre_gb_pitch * idx_tensor, 1).cpu() * binwidth - 180
            pre_gb_yaw = torch.sum(pre_gb_yaw * idx_tensor, 1).cpu() * binwidth - 180
            pre_gr_pitch = torch.sum(pre_gr_pitch * idx_tensor, 1).cpu() * binwidth - 180
            pre_gr_yaw = torch.sum(pre_gr_yaw * idx_tensor, 1).cpu() * binwidth - 180
