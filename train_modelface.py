...
import datasets
from model_face import FaceRes
...
def get_ignored_params(model):
  ...
  b = [model.face_res.conv1, model.face_res.bn1]
...
def get_non_ignored_params(model):
  ...
  b = [model.face_res.layer1, model.face_res.layer2, model.face_res.layer3, model.face_res.layer4]
  for i in range(len(b)):
    for module_name, module in b[i].named_modules():
      if 'bn' in module_name:
        module.eval()
...
def getArch_weights(arch, bins):
  if arch == 'ResNet18':
    model = FaceRes(models.resnet18(pretrained=True), bins)
  elif arch == 'ResNet34':
    model = FaceRes(models.resnet34(pretrained=True), bins)
  elif arch == 'ResNet101':
    model = FaceRes(models.resnet101(pretrained=True), bins)
  elif arch == 'ResNet152':
    model = FaceRes(models.resnet152(pretrained=True), bins)
  else:
    model = FaceRes(models.resnet50(pretrained=True), bins)
...
if __name__=='__main__':
  ...
  if dataset=="gaze360":
    ...
    summary_name = '{}_{}'.format('FaceRes-gaze360', str(today.strftime('%Y-%-m*%-d_%-H*%-M*%-S')))
