...
def getArch_weights(arch, bins):
  if arch == 'ResNet18':
    model = Res(models.resnet18(pretrained=True), bins)
  elif arch == 'ResNet34':
    model = Res(models.resnet34(pretrained=True), bins)
  elif arch == 'ResNet101':
    model = Res(models.resnet101(pretrained=True), bins)
  elif arch == 'ResNet152':
    model = Res(models.resnet152(pretrained=True), bins)
  else:
    model = Res(models.resnet50(pretrained=True), bins)
